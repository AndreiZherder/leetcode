...
nc.find(10); // Number 10 is at the indices 2, 3, and 5. The smallest index that is filled with 10 is 2.
...
class NumberContainers:
    ...
    def __init__(self):
        self.indexes_of_number = defaultdict(lambda: SortedList())
        self.indexes = dict()

    def change(self, index: int, number: int) -> None:
        if index in self.indexes:
            old_number = self.indexes[index]
            self.indexes_of_number[old_number].discard(index)
        self.indexes[index] = number
        self.indexes_of_number[number].add(index)

    def find(self, number: int) -> int:
        if self.indexes_of_number[number]:
            return self.indexes_of_number[number][0]
        else:
            return -1
