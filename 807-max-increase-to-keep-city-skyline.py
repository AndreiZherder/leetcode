...
square prism. You are given a 0-indexed n x n integer matrix grid where grid[r][c] represents the height
...
class Solution:
    def maxIncreaseKeepingSkyline(self, grid: List[List[int]]) -> int:
        n = len(grid)
        m = len(grid)
        row_mxs = [-10 ** 20 for i in range(n)]
        col_mxs = [-10 ** 20 for j in range(m)]
        for i in range(n):
            for j in range(m):
                row_mxs[i] = max(row_mxs[i], grid[i][j])
                col_mxs[j] = max(col_mxs[j], grid[i][j])
        ans = 0
        for i in range(n):
            for j in range(m):
                ans += max(0, min(row_mxs[i], col_mxs[j]) - grid[i][j])
        return ans
