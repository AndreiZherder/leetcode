...
An edge reversal changes the direction of an edge, i.e.,
a directed edge going from node ui to node vi becomes a directed edge going from node vi to node ui.

For every node i in the range [0, n - 1],
your task is to independently calculate the minimum number of edge reversals required
so it is possible to reach any other node starting from node i through a sequence of directed edges.

Return an integer array answer,
where answer[i] is the minimum number of edge reversals required
so it is possible to reach any other node starting from node i through a sequence of directed edges.
...
class Solution:
    def minEdgeReversals(self, n: int, edges: List[List[int]]) -> List[int]:
        def dfs1(v: int, par: int) -> int:
            parent[v] = par
            for u in g[v]:
                if u != par:
                    ans1[v] += dfs1(u, v) + (0 if (v, u) in e else 1)
            return ans1[v]

        def dfs2(v: int, par: int):
            if v == 0:
                ans2[v] = ans1[v]
            else:
                ans2[v] = ans2[parent[v]] + (-1 if (v, parent[v]) in e else 1)
            for u in g[v]:
                if u != par:
                    dfs2(u, v)
