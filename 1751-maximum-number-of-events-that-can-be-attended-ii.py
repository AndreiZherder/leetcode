...
and ends at endDayi, and if you attend this event, you will receive a value of valuei.
...
Note that the end day is inclusive: that is, you cannot attend two events where one of them starts
...
Example 3:Input: events = [[1,1,1],[2,2,2],[3,3,3],[4,4,4]], k = 3
Output: 9
    ...
    def maxValue(self, events: List[List[int]], k: int) -> int:
        ...
        def dp(i: int, k: int) -> int:
            ...
            if k == 0:
                return 0
            return max(dp(i + 1, k),
                       dp(bisect_right(events, [events[i][1], 10 ** 20, 10 ** 20]), k - 1) + events[i][2])
