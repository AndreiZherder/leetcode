"""
Some robots are standing on an infinite number line with their initial coordinates given by a 0-indexed integer array
nums and will start moving once given the command to move. The robots will move a unit distance each second.
...
Output: 8
Explanation:
After 1 second, the positions are [-1,-1,1]. Now, the robot at index 0 will move left,
and the robot at index 1 will move right.
After 2 seconds, the positions are [-2,0,0]. Now, the robot at index 1 will move left,
and the robot at index 2 will move right.
...
class Solution:
    def sumDistance(self, nums: List[int], s: str, d: int) -> int:
        n = len(nums)
        a = []
        for num, direction in zip(nums, s):
            if direction == 'L':
                a.append(num - d)
            else:
                a.append(num + d)
...
def main():
    ...
    s = "RLL"
    d = 3
    print(Solution().sumDistance(nums, s, d))
