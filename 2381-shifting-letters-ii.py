...
(wrapping around so that 'z' becomes 'a'). Similarly, shifting a character backward means replacing it with
...
class Solution:
    def shiftingLetters(self, s: str, shifts: List[List[int]]) -> str:
        ...
        for i, j, direction in shifts:
            ...
            diff[i] += direction
            diff[j + 1] -= direction
        add = list(accumulate(diff))
        nums = [(a + b) % 26 for a, b in zip(nums, add)]
