...
Each element in nums represents an ID, and the corresponding element in freq indicates
how many times that ID should be added to or removed from the collection at each step.

Addition of IDs: If freq[i] is positive, it means freq[i] IDs with the value nums[i]
are added to the collection at step i.
Removal of IDs: If freq[i] is negative, it means -freq[i] IDs with the value nums[i]
...
class Solution:
    def mostFrequentIDs(self, nums: List[int], freq: List[int]) -> List[int]:
        ...
        for num, f in zip(nums, freq):
            vsd[num] = vsd.get(num, 0) + f
            ans.append(vsd.peekitem(-1)[1])
        return ans
