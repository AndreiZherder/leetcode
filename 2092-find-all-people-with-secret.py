...
class Solution:
    def findAllPeople(self, n: int, meetings: List[List[int]], firstPerson: int) -> List[int]:
        ...
        for v, u, t in meetings:
            events[t].append((v, u))
            meets[t].add(v)
            meets[t].add(u)
        dsu_all = DSU(n)
        dsu_all.union(0, firstPerson)
        for t in sorted(events):
            dsu = DSU(len(meets[t]))
            i_to_v = dict()
            v_to_i = dict()
            for i, v in enumerate(meets[t]):
                i_to_v[i] = v
                v_to_i[v] = i
            for v, u in events[t]:
                dsu.union(v_to_i[v], v_to_i[u])
            components = defaultdict(set)
            for v in meets[t]:
                components[i_to_v[dsu.find(v_to_i[v])]].add(v)
            for i, vs in components.items():
                for v in vs:
                    if dsu_all.find(v) == dsu_all.find(0):
                        for v in vs:
                            dsu_all.union(v, 0)
                        break
        ans = []
