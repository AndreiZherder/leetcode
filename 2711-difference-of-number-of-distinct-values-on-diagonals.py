...
the topmost row or leftmost column and going in the bottom-right direction until reaching the matrix's end.

A cell (r1, c1) belongs to the top-left diagonal of the cell (r, c), if both belong to the same diagonal and r1 < r.
...
The 4th diagram denotes a grid for cell (1,1), where blue-colored cells are cells on its bottom-right diagonal and
red-colored cells are cells on its top-left diagonal.
...
Input: grid = [[1]]
Output: [[0]]
Explanation: - The cell (0,0) contains [] on its bottom-right diagonal and [] on its top-left diagonal.
The answer is |0 - 0| = 0.
    ...
    def differenceOfDistinctValues(self, grid: List[List[int]]) -> List[List[int]]:
        ...
        for i in range(n):
            for j in range(m):
                ...
                y, x = i - 1, j - 1
                while y >= 0 and x >= 0:
                    topleft.add(grid[y][x])
                    y -= 1
                    x -= 1
                y, x = i + 1, j + 1
                while y < n and x < m:
                    bottomright.add(grid[y][x])
                    y += 1
                    x += 1
