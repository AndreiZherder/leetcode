"""
ou are given a 0-indexed array nums consisting of positive integers, representing targets on a number line.
...
with values that can be represented as nums[i] + c * space, where c is any non-negative integer.
...
class Solution:
    def destroyTargets(self, nums: List[int], space: int) -> int:
        ...
        for v in sorted(d.values(), key=lambda x: (-len(x), min(x))):
            return min(v)
