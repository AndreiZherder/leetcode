...
class FoodRatings:
    ...
    def __init__(self, foods: List[str], cuisines: List[str], ratings: List[int]):
        self.food_cousine = dict()
        self.cousine_food = defaultdict(lambda: ValueSortedDict())
        for f, c, r in zip(foods, cuisines, ratings):
            self.food_cousine[f] = c
            self.cousine_food[c][f] = (-r, f)

    def changeRating(self, food: str, newRating: int) -> None:
        self.cousine_food[self.food_cousine[food]][food] = (-newRating, food)

    def highestRated(self, cuisine: str) -> str:
        return self.cousine_food[cuisine].peekitem(0)[1][1]
