...
class UndergroundSystem:
    ...
    def checkOut(self, id: int, stationName: str, t: int) -> None:
        first_station, start = self.checkins.pop(id)
        if (first_station, stationName) not in self.routes:
            self.routes[(first_station, stationName)] = {'duration': t - start, 'n': 1}
        else:
            self.routes[(first_station, stationName)]['duration'] += t - start
            self.routes[(first_station, stationName)]['n'] += 1

    def getAverageTime(self, startStation: str, endStation: str) -> float:
        route = self.routes[(startStation, endStation)]
        return route['duration'] / route['n']
