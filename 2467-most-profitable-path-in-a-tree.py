...
class Solution:
    def mostProfitablePath(self, edges: List[List[int]], bob: int, amount: List[int]) -> int:
        ...
        def dfs(node: int, from_start: int, visited: List[bool]) -> Tuple[int, int]:
            best = -float('Inf')
            from_node_to_bob = 0
            visited[node] = True
            for kid in tree[node]:
                if not visited[kid]:
                    cur, to_bob = dfs(kid, from_start + 1, visited)
                    if not from_node_to_bob:
                        from_node_to_bob = to_bob
                    best = max(best, cur)
            ...
            if node == bob:
                return best, 1
            elif from_node_to_bob > 0:
                if from_start > from_node_to_bob:
                    return best, from_node_to_bob + 1
                elif from_start == from_node_to_bob:
                    return best + amount[node] // 2, from_node_to_bob + 1
                else:
                    return best + amount[node], from_node_to_bob + 1
            else:
                return best + amount[node], 0
