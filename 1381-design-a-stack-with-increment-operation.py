...
class CustomStack:
    ...
    def pop(self) -> int:
        if self.stack:
            x, add = self.stack.pop()
            if self.stack:
                prev, prev_add = self.stack[-1]
                self.stack[-1] = (prev, prev_add + add)
            return x + add
        else:
            return -1
    ...
    def increment(self, k: int, val: int) -> None:
        if self.stack:
            prev, prev_add = self.stack[min(k - 1, len(self.stack) - 1)]
            self.stack[min(k - 1, len(self.stack) - 1)] = (prev, prev_add + val)
