...
class Solution:
    def checkValidCuts(self, n: int, rectangles: List[List[int]]) -> bool:
        def check(segments: List[List[int]]) -> bool:
            segments.sort()
            max_end = segments[0][0] + 1
            cnt = 0
        ...
        return check([[sx, ex] for sx, sy, ex, ey in rectangles]) or check([[sy, ey] for sx, sy, ex, ey in rectangles])
