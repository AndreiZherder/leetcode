...
but nums[2] has the smaller index. Hence, we multipy the score by nums[2]. The score becomes 342 * 14 = 4788.
...
class Solution:
    def maximumScore(self, nums: List[int], k: int) -> int:
        ...
        def prevGreaterOrEqualElements(nums: List[int]) -> List[int]:
            ans = []
            n = len(nums)
            stack = []
            for i in range(n):
        ...
        mod = 10 ** 9 + 7
        n = len(nums)
        score = [len(set(prime_factors(num))) for num in nums]
        lefts = prevGreaterOrEqualElements(score)
        rights = nextGreaterElements(score)
        indexes = sorted(range(n), key=lambda i: (-nums[i], -score[i], i))
        ans = 1
        for i in indexes:
            left = lefts[i]
            right = rights[i]
