...
For each query i, compute the minimum absolute difference of the subarray nums[li...ri] containing the elements of nums
...
class Solution:
    def minDifference(self, nums: List[int], queries: List[List[int]]) -> List[int]:
        ...
        for l, r in queries:
            a = []
            for num in d:
                i = bisect_left(d[num], l)
                j = bisect_right(d[num], r) - 1
                if i <= j:
                    a.append(num)
            if len(a) < 2:
                ans.append(-1)
            ...
            for num in a[1:]:
                if num != prev:
                    best = min(best, num - prev)
                    prev = num
            ans.append(best if best != 10 ** 20 else -1)
        return ans
