...
its path. For all cells (r, c) traversed on the path, grid[r][c] is set to 0.
...
class Solution:
    def gridGame(self, grid: List[List[int]]) -> int:
        n = len(grid[0])
        pref1 = list(accumulate(grid[0], initial=0))
        pref2 = list(accumulate(grid[1], initial=0))
        best = 10 ** 20
        for i in range(n):
            best = min(best, max(pref1[n] - pref1[i + 1], pref2[i]))
        return best
