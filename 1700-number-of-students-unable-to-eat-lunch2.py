...
in the stack (i = 0 is the top of the stack) and students[j] is the preference of the jth student in the initial queue
...
class Solution:
    def countStudents(self, students: List[int], sandwiches: List[int]) -> int:
        ...
        q = deque()
        while students:
            length1 = len(students)
            while students:
                if students[0] == sandwiches[0]:
                    students.popleft()
                    sandwiches.popleft()
                else:
                    q.append(students.popleft())
                length2 = len(q)
                if length1 == length2:
                    return length1
            students, q = q, students
        return 0
