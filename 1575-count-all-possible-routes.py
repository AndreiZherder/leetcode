...
Output: 0
Explanation: It is impossible to get from 0 to 2 using only 3 units of fuel since the shortest route needs
4 units of fuel.
...
class Solution:
    def countRoutes(self, locations: List[int], start: int, finish: int, fuel: int) -> int:
        ...
        @lru_cache(None)
        def dp(i: int, fuel_remain: int) -> int:
            if fuel_remain > fuel:
                ans = 0
            elif i == start and fuel_remain == fuel:
                ans = 1
            else:
                ans = 0
                for j in range(n):
                    if j != i:
                        ans = (ans + dp(j, fuel_remain + abs(locations[j] - locations[i]))) % mod
            return ans

        ans = 0
        for fuel_remain in range(fuel + 1):
            ans = (ans + dp(finish, fuel_remain)) % mod
        return ans
