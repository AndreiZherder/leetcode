...
class Solution:
    def getOrder(self, tasks: List[List[int]]) -> List[int]:
        ...
        tasks.sort()
        end_time = tasks[0][0]
        for next_enqueue_time, next_processing_time, i in tasks:
            while next_enqueue_time > end_time:
                if h:
                    cur_processing_time, j = heappop(h)
                    ans.append(j)
                    end_time += cur_processing_time
                else:
                    end_time = next_enqueue_time
            heappush(h, (next_processing_time, i))
        while h:
            cur_processing_time, j = heappop(h)
            ans.append(j)
        return ans
