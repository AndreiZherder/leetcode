...
class Solution:
    def leftmostBuildingQueries(self, heights: List[int], queries: List[List[int]]) -> List[int]:
        ...
        q = sorted(((x, y, i) for i, (x, y) in enumerate(queries)), key=lambda z: (-z[1], -z[0]))
        ans = [-1 for i in range(m)]
        sl = SortedList()
        right = n
        for x, y, i in q:
            for j in range(right - 1, y - 1, -1):
                while sl and sl[0][0] <= heights[j]:
                    sl.pop(0)
                sl.add((heights[j], j))
            if x == y:
                ans[i] = x
            else:
                k = sl.bisect_left((max(heights[x] + 1, heights[y]), 0))
                if k != len(sl):
                    ans[i] = sl[k][1]
            right = y
        return ans
