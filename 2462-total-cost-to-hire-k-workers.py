...
"""
from heapq import nsmallest, heappush, heapreplace, heappop
    ...
    def totalCost(self, costs: List[int], k: int, candidates: int) -> int:
        ...
        if n <= candidates * 2:
            ...
            for i in range(candidates):
                heappush(lh, costs[i])
            for i in range(candidates):
                heappush(rh, costs[n - i - 1])
            for i in range(k):
                if not rh or (lh and lh[0] <= rh[0]):
                    if left <= right:
                        ans += heapreplace(lh, costs[left])
                        left += 1
                    else:
                        ans += heappop(lh)
                else:
                    if left <= right:
                        ans += heapreplace(rh, costs[right])
                        right -= 1
                    else:
                        ans += heappop(rh)
        return ans
