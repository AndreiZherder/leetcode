...
Tests are generated such that the answer will not exceed 107 and hour will have at most two digits after
the decimal point.
...
class Solution:
    def minSpeedOnTime(self, dist: List[int], hour: float) -> int:
        ...
        while left <= right:
            ...
            if sum(ceil(d, mid) for d in dist[:n - 1]) + dist[n - 1] / mid <= hour:
                right = mid - 1
            else:
                left = mid + 1
        return left if left <= mx else -1
