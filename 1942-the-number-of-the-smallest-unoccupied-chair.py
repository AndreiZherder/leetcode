...
class Solution:
    def smallestChair(self, times: List[List[int]], targetFriend: int) -> int:
        ...
        for i, (arrival, leaving) in enumerate(times):
            t.append((arrival, 2, i))
            t.append((leaving, 1, i))
        t.sort()
        occupied = dict()
        for time, event, i in t:
            if event == 2:
                if not available_seats:
                    occupied[i] = not_used_seat
                    not_used_seat += 1
                else:
                    occupied[i] = heappop(available_seats)
                if i == targetFriend:
                    return occupied[i]
            else:
                heappush(available_seats, occupied[i])
