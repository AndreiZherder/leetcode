...
class Solution:
    def longestUnivaluePath(self, root: Optional[TreeNode]) -> int:
        def dfs(root: TreeNode) -> (int, int):
            ...
            ans_right = 0
            if root.left:
                depth_left, ans_left = dfs(root.left)
                if root.left.val != root.val:
                    depth_left = 0
            if root.right:
                depth_right, ans_right = dfs(root.right)
                if root.right.val != root.val:
                    depth_right = 0
            depth = 1 + max(depth_left, depth_right)
            return depth, max(ans_left, ans_right, depth_left + depth_right + 1)
