...
class Solution:
    def getLengthOfOptimalCompression(self, s: str, k: int) -> int:
        @lru_cache(None)
        def dp(i: int, prev: str, length: int, k: int) -> int:
            def code(length: int) -> int:
                if length == 0:
                    return 0
                if length == 1:
                    return 1
            ...
            if k == 0:
                if s[i] == prev:
                    ans = min(ans, dp(i + 1, s[i], length + 1, k))
                else:
                    ans = min(ans, code(length) + dp(i + 1, s[i], 1, k))
            else:
                if s[i] == prev:
                    ans = min(ans, dp(i + 1, s[i], length + 1, k))
                    ans = min(ans, dp(i + 1, s[i], length, k - 1))
                else:
                    ans = min(ans, code(length) + dp(i + 1, s[i], 1, k))
                    ans = min(ans, dp(i + 1, prev, length, k - 1))
            return ans
