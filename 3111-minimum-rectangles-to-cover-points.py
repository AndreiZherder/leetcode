...
Return an integer denoting the minimum number of rectangles needed
so that each point is covered by at least one rectangle.
...
class Solution:
    def minRectanglesToCoverPoints(self, points: List[List[int]], w: int) -> int:
        ...
        while points:
            start = points[-1][0]
            ans += 1
            while points and start - w <= points[-1][0]:
                points.pop()
        return ans
