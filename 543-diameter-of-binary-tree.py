...
class Solution:
    def diameterOfBinaryTree(self, root: Optional[TreeNode]) -> int:
        def dfs(root: Optional[TreeNode]) -> Tuple[int, int]:
            if not root:
                return 0, 0
            length_left, d_left = dfs(root.left)
            length_right, d_right = dfs(root.right)
            length = 1 + max(length_left, length_right)
            d = max(d_left, d_right, length_left + length_right + 1)
            return length, d
        return dfs(root)[1] - 1
