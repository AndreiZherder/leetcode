...
is the number of new customers arriving just before the ith rotation (0-indexed).
...
to get all the customers down safely. Note that if there are currently more than four customers waiting at the wheel,
    ...
    def minOperationsMaxProfit(self, customers: List[int], boardingCost: int, runningCost: int) -> int:
        ...
        while i < n or onboard:
            ...
            if i < n:
                ...
                x = 0
            if waiting:
                boarding_from_queue = min(waiting, 4)
            else:
                boarding_from_queue = 0
            boarding_from_newcustomers = min(x, 4 - boarding_from_queue)
            waiting += x - boarding_from_newcustomers - boarding_from_queue
            wheel.append(boarding_from_queue + boarding_from_newcustomers)
            onboard += wheel[-1]
            cur += 1
            total += (boarding_from_queue + boarding_from_newcustomers) * boardingCost - runningCost
            if total > 0 and total > best:
                best = total
                ans = cur
            i += 1
        return ans
