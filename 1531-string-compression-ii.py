...
class Solution:
    def getLengthOfOptimalCompression(self, s: str, k: int) -> int:
        def encoded_length(length: int) -> int:
            if length == 0:
                return 0
            if length == 1:
                return 1
        ...
        @lru_cache(None)
        def dfs(i: int, prev_char: str, length: int, k: int) -> int:
            if i == n:
                return encoded_length(length)
            ans = float('Inf')
            if s[i] == prev_char:
                ans = min(ans, dfs(i + 1, prev_char, length + 1, k))
                if k > 0:
                    ans = min(ans, dfs(i + 1, prev_char, length, k - 1))
            else:
                ans = min(ans, dfs(i + 1, s[i], 1, k) + encoded_length(length))
                if k > 0:
                    ans = min(ans, dfs(i + 1, prev_char, length, k - 1))
            return ans
