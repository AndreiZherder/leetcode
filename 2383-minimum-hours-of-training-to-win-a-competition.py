...
Output: 8
Explanation: You can increase your energy to 11 after 6 hours of training, and your experience to 5 after 2 hours
of training.
...
class Solution:
    def minNumberOfHours(self, initialEnergy: int, initialExperience: int, energy: List[int],
        ...
        ans = 0
        my_energy, my_exp = initialEnergy, initialExperience
        for energy, exp in zip(energy, experience):
            if energy >= my_energy:
                ans += energy - my_energy + 1
                my_energy = energy + 1
            if exp >= my_exp:
                ans += exp - my_exp + 1
                my_exp = exp + 1
