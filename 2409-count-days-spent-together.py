...
class Solution:
    def countDaysTogether(self, arriveAlice: str, leaveAlice: str, arriveBob: str, leaveBob: str) -> int:
        ...
        acc = list(accumulate(days, initial=0))

        montha, daya = (int(num) for num in arriveAlice.split('-'))
        monthb, dayb = (int(num) for num in arriveBob.split('-'))
        arrive = max(acc[montha - 1] + daya, acc[monthb - 1] + dayb)

        montha, daya = (int(num) for num in leaveAlice.split('-'))
        monthb, dayb = (int(num) for num in leaveBob.split('-'))
        leave = min(acc[montha - 1] + daya, acc[monthb - 1] + dayb)
