...
class Solution:
    def mincostTickets(self, days: List[int], costs: List[int]) -> int:
        day_cost, week_cost, month_cost = costs
        n = max(days) + 1 + 30
        ...
        for i in range(n):
            if i + 1 < n:
                if a[i + 1]:
                    dp[i + 1] = min(dp[i + 1], dp[i] + day_cost)
                else:
                    dp[i + 1] = min(dp[i + 1], dp[i])
            if i + 7 < n:
                if pref[i + 7] - pref[i]:
                    dp[i + 7] = min(dp[i + 7], dp[i] + week_cost)
                else:
                    dp[i + 7] = min(dp[i + 7], dp[i])
            if i + 30 < n:
                if pref[i + 30] - pref[i]:
                    dp[i + 30] = min(dp[i + 30], dp[i] + month_cost)
                else:
                    dp[i + 30] = min(dp[i + 30], dp[i])
        return dp[n - 1]
