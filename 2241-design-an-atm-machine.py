...
class ATM:
    ...
    def withdraw(self, amount: int) -> List[int]:
        ...
        while amount and i >= 0:
            withdrawn = amount // banknotes[i]
            withdrawn = min(withdrawn, self.bank[i])
            ans[i] = withdrawn
            self.bank[i] -= withdrawn
            amount -= withdrawn * banknotes[i]
            i -= 1
        if i < 0 and amount:
            self.bank = self.backup
            ans = [-1]
        return ans
