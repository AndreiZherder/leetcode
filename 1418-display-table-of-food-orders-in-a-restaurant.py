...
alphabetical order. The first row should be a header whose first column is “Table”,
...
class Solution:
    def displayTable(self, orders: List[List[str]]) -> List[List[str]]:
        ...
        for table in sorted(c):
            ans.append([str(table)] + [str(c[table][food[i]]) for i in range(len(food))])
        return ans
