...
Return an array answer of size m where answer[i] is the minimum number of operations to make all elements of nums
equal to queries[i].
...
class Solution:
    def minOperations(self, nums: List[int], queries: List[int]) -> List[int]:
        ...
        for query in queries:
            i = bisect_right(nums, query) + 1
            left_sum = pref[i - 1]
            right_sum = pref[-1] - pref[i - 1]
            ans.append(query * (i - 1) - left_sum + right_sum - query * (n - i + 1))
        return ans
