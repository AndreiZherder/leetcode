...
the vertex that comes before it and the vertex that comes after it.
...
traversed during the walk. In other words, if the sequence of edge weights encountered
during the walk is w0, w1, w2, ..., wk, then the cost is calculated as w0 & w1 & w2 & ... & wk,
...
class DSU:
    ...
    def cost_of_walk(self, id: int) -> int:
        return self.min_cost[id]

    def union(self, i: int, j: int, c: int):
        id1 = self.find(i)
        id2 = self.find(j)
        if id1 == id2:
            self.min_cost[id1] &= c
        ...
        if self.rank[id1] > self.rank[id2]:
            self.parent[id2] = id1
            self.min_cost[id1] &= self.min_cost[id2] & c
        else:
            self.parent[id1] = id2
            self.min_cost[id2] &= self.min_cost[id1] & c
            if self.rank[id1] == self.rank[id2]:
                self.rank[id2] += 1
...
class Solution:
    def minimumCost(self, n: int, edges: List[List[int]], query: List[List[int]]) -> List[int]:
        ...
        for v, u in query:
            ...
            id2 = dsu.find(u)
            if id1 == id2:
                ans.append(dsu.cost_of_walk(id1))
            else:
                ans.append(-1)
        return ans
